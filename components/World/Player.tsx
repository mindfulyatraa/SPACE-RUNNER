...
export const Player: React.FC = () => {
    ...
    // Memoized Materials - Switched to Lambert for Performance
    const { armorMaterial, jointMaterial, glowMaterial, shadowMaterial } = useMemo(() => {
        ...
        return {
            armorMaterial: new THREE.MeshLambertMaterial({ color: armorColor }),
            jointMaterial: new THREE.MeshLambertMaterial({ color: '#111111' }),
        ...
        if (showFlicker) {
            if (isInvincible.current) {
                if (Date.now() - lastDamageTime.current > 1500) {
                    ...
                    // Optimized flicker using time-based calculation instead of constant Date.now() calls
                    groupRef.current.visible = Math.floor((Date.now() - lastDamageTime.current) / 100) % 2 === 0;
                }
            }
